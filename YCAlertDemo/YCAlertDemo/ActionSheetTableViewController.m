...
- (void)viewDidLoad {
    ...
    [baseArray addObject:@{@"title" : @"显示一个自定义action的 actionSheet 菜单" , @"content" : @"action的自定义属性可查看\"YCAction\"类."}];
...
//                custom.isAutoWidth = YES; // 设置自动宽度后 会根据insets和最大的宽度自动计算自定义视图的宽度 并修改其frame属性
        ...
        case 4:
        {
            ...
            //.YcMaxWidth(280) // YCMaxWidth设置的最大宽度为固定数值 横屏竖屏都会采用这个宽度 (高度同理)
            .YcConfigMaxWidth(^CGFloat (YCScreenOrientationType type) { // YCConfigMaxWidth可以单独对横屏竖屏情况进行设置
        ...
        {
            [YCAlert actionsheet].config
            .YcTitle(@"这是一个actionSheet 它有三个不同类型的action!")
            .YcAction(@"一个默认action", ^{
        ...
        {
            [YCAlert actionsheet].config
            .YcTitle(@"自定义 Action 的 actionSheet!")
        ...
        {
            [YCAlert actionsheet].config
            .YcTitle(@"这是一个毛玻璃背景样式的actionSheet")
        ...
        case 13:
        {
            ...
            [YCAlert actionsheet].config
            .YcTitle(@"自定义动画样式的 Actionsheet")
            .YcContent(@"动画样式可设置动画方向, 淡入淡出, 缩放等")
            .YcAction(@"好的", ^{
                
                // 点击事件Block
            })
            //.YcOpenAnimationStyle(YCAnimationStyleOrientationTop | YCAnimationStyleZoomShrink) //这里设置打开动画样式的方向为上 以及淡入效果.
            //.YcCloseAnimationStyle(YCAnimationStyleOrientationBottom | YCAnimationStyleZoomShrink) //这里设置关闭动画样式的方向为下 以及淡出效果
            .YcOpenAnimationStyle(YCAnimationStyleOrientationLeft | YCAnimationStyleFade) //这里设置打开动画样式的方向为左 以及缩放效果.
            .YcCloseAnimationStyle(YCAnimationStyleOrientationRight | YCAnimationStyleFade) //这里设置关闭动画样式的方向为右 以及缩放效果
            .YcShow();
        }
            break;
        ...
        case 14:
        {
            ...
            //            .YcCornerRadius(20)  // 优先级低于YCCornerRadii
...
- (void)demo:(NSInteger)index{
    ...
    switch (index) {
        ...
        {
            [YCAlert actionsheet].config
            .YcContent(@"退出后不会通知群聊中其他成员, 且不会接收此群聊消息.出后不会通知群聊中其他成员, 且不会接收此群聊消息")
            .YcDestructiveAction(@"确定", ^{
            ...
            .YcActionSheetCancelActionSpaceColor([UIColor colorWithWhite:0.92 alpha:1.0f]) // 设置取消按钮间隔的颜色
            .YcActionSheetBottomMargin(0.0f) // 设置底部距离屏幕的边距为0
            .YcCornerRadius(0.0f) // 设置圆角曲率为0
            ...
            .YcActionSheetBackgroundColor([UIColor whiteColor]) // 通过设置背景颜色来填充底部间隙
            .YcShow();
        }
            break;
        ...
        case 3:
        {
            ...
            .YcActionSheetBottomMargin(0.0f)
            .YcActionSheetBackgroundColor([UIColor whiteColor])
            .YcCornerRadius(0.0f)
            .YcConfigMaxWidth(^CGFloat(YCScreenOrientationType type) {
                
                // 这是最大宽度为屏幕宽度 (横屏和竖屏)
                
                return type == YCScreenOrientationTypeHorizontal ? CGRectGetHeight([[UIScreen mainScreen] bounds]) : CGRectGetWidth([[UIScreen mainScreen] bounds]);
            })
            .YcShow();
        }
            break;
        ...
        case 4:
        {
            ...
            .YcHeaderColor([UIColor colorWithRed:243/255.0f green:243/255.0f blue:243/255.0f alpha:1.0f])
            .YcActionSheetBottomMargin(0.0f) // 设置底部距离屏幕的边距为0
            .YcActionSheetBackgroundColor([UIColor whiteColor])
            .YcCornerRadius(0.0f) // 设置圆角曲率为0
            .YcConfigMaxWidth(^CGFloat(YCScreenOrientationType type) {
            ...
            view.openMoreBlock = ^(MoreType type) {
                ...
                switch (type) {
                    ...
                    case MoreTypeToTheme:
                        ...
                        // 切换主题 (关于主题开源库 推荐一下: https://github.com/lixiang1994/YCTheme)
