...
//  Copyright © 2019 Codyy.YCY. All rights reserved.
//
...
typedef NS_ENUM(NSInteger, YCBackgroundStyle) {
    /** 背景样式 模糊 */
    YCBackgroundStyleBlur,
    /** 背景样式 半透明 */
...
@property (nonatomic , copy ) CGFloat (^modelMaxWidthBlock)(YCScreenOrientationType);
@property (nonatomic , copy ) CGFloat (^modelMaxHeightBlock)(YCScreenOrientationType);
...
@property (nonatomic , assign ) YCBackgroundStyle modelBackgroundStyle;
...
- (YCConfigToString)YcTitle{
    
    return ^(NSString *str){
        
        return self.YcAddTitle(^(UILabel *label) {
...
- (YCConfigToString)YcContent{
    
    return ^(NSString *str){
        
        return  self.YcAddContent(^(UILabel *label) {
...
- (YCConfigToView)YcCustomView{
    
    return ^(UIView *view){
        
        return self.YcAddCustomView(^(YCCustomView *custom) {
...
- (YCConfigToStringAndBlock)YcAction{
    
    return ^(NSString *title , void(^block)(void)){
        
        return self.YcAddAction(^(YCAction *action) {
...
- (YCConfigToStringAndBlock)YcCancelAction{
...
- (YCConfigToStringAndBlock)YcDestructiveAction{
    
    return ^(NSString *title , void(^block)(void)){
        
        return self.YcAddAction(^(YCAction *action) {
            
            action.type = YCActionTypeDestructive;
...
- (YCConfigToConfigLabel)YcAddTitle{
    
    return ^(void(^block)(UILabel *)){
        
        return self.YcAddItem(^(YCItem *item) {
            
            item.type = YCItemTypeTitle;
...
- (YCConfigToConfigLabel)YcAddContent{
    
    return ^(void(^block)(UILabel *)){
        
        return self.YcAddItem(^(YCItem *item) {
            
            item.type = YCItemTypeContent;
...
- (YCConfigToCustomView)YcAddCustomView{
    
    return ^(void(^block)(YCCustomView *custom)){
        
        return self.YcAddItem(^(YCItem *item) {
            
            item.type = YCItemTypeCustomView;
...
- (YCConfigToItem)YcAddItem{
    
    return ^(void(^block)(YCItem *)){
...
- (YCConfigToAction)YcAddAction{
    
    return ^(void(^block)(YCAction *)){
...
- (YCConfigToEdgeInsets)YcHeaderInsets{
...
- (YCConfigToEdgeInsets)YcItemInsets{
...
- (YCConfigToFloat)YcMaxWidth{
    
    return ^(CGFloat number){
        
        return self.YcConfigMaxWidth(^CGFloat(YCScreenOrientationType type) {
...
- (YCConfigToFloat)YcMaxHeight{
    
    return ^(CGFloat number){
        
        return self.YcConfigMaxHeight(^CGFloat(YCScreenOrientationType type) {
...
- (YCConfigToFloatBlock)YcConfigMaxWidth{
    
    return ^(CGFloat(^block)(YCScreenOrientationType type)){
...
- (YCConfigToFloatBlock)YcConfigMaxHeight{
...
- (YCConfigToFloat)YcCornerRadius{
...
- (YCConfigToCornerRadii)YcCornerRadii{
...
- (YCConfigToFloat)YcOpenAnimationDuration{
...
- (YCConfigToFloat)YcCloseAnimationDuration{
...
- (YCConfigToColor)YcBackGroundColor{
...
- (YCConfigToFloat)YcBackgroundStyleTranslucent{
    
    return ^(CGFloat number){
        
        self.modelBackgroundStyle = YCBackgroundStyleTranslucent;
...
- (YCConfigToBlurEffectStyle)YcBackgroundStyleBlur{
    
    return ^(UIBlurEffectStyle style){
        
        self.modelBackgroundStyle = YCBackgroundStyleBlur;
...
- (YCConfigToBool)YcClickHeaderClose{
...
- (YCConfigToBool)YcClickBackgroundClose{
...
- (YCConfigToBool)YcIsScrollEnabled{
...
- (YCConfigToFloat)YcShadowOpacity{
...
- (YCConfigToFloat)YcShadowRadius{
...
- (YCConfigToBool)YcContinueQueueDisplay{
...
- (YCConfigToBool)YcShouldAutorotate{
...
- (YCConfigToInterfaceOrientationMask)YcSupportedInterfaceOrientations{
...
- (YCConfigToBlockAndBlock)YcOpenAnimationConfig{
...
- (YCConfigToBlockAndBlock)YcCloseAnimationConfig{
...
- (YCConfigToAnimationStyle)YcOpenAnimationStyle{
    
    return ^(YCAnimationStyle style){
...
- (YCConfigToAnimationStyle)YcCloseAnimationStyle{
...
- (YCConfigToStatusBarStyle)YcStatusBarStyle{
...
- (YCConfigToPoint)YcAlertCenterOffset {
...
- (YCConfigToBool)YcAvoidKeyboard{
...
- (YCConfigToColor)YcActionSheetCancelActionSpaceColor{
...
- (YCConfigToFloat)YcActionSheetBottomMargin{
...
- (YCConfigToColor)YcActionSheetBackgroundColor{
...
@protocol YCAlertProtocol <NSObject>
...
+ (YCAlert *)shareManager{
    
    static YCAlert *alertManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        alertManager = [[YCAlert alloc] init];
    });
...
+ (YCAlertWindow *)getAlertWindow{
    ...
    return [YCAlert shareManager].ycWindow;
}

+ (void)configMainWindow:(UIWindow *)window{
    
    if (window) [YCAlert shareManager].mainWindow = window;
}

+ (void)continueQueueDisplay{
    
    if ([YCAlert shareManager].queueArray.count) [YCAlert shareManager].queueArray.lastObject.config.modelFinishConfig();
}
...
+ (void)closeWithIdentifier:(NSString *)identifier force:(BOOL)force completionBlock:(void (^)(void))completionBlock{
    ...
    if ([YCAlert shareManager].queueArray.count) {
        ...
        NSUInteger count = [YCAlert shareManager].queueArray.count;
        ...
        for (NSUInteger i = 0; i < count; i++) {
            ...
            if (model.modelIdentifier != nil && [identifier isEqualToString: model.modelIdentifier]) {
                ...
                if (i == count - 1 && [[YCAlert shareManager] viewController]) {
                    if (force) {
        ...
        [[YCAlert shareManager].queueArray removeObjectsAtIndexes:indexs];
        
        if (isLast) {
        
            [YCAlert closeWithCompletionBlock:completionBlock];
...
- (YCAlertWindow *)ycWindow{
    ...
    if (!_ycWindow) {
        ...
        _ycWindow.backgroundColor = [UIColor clearColor];
        
        _ycWindow.windowLevel = UIWindowLevelAlert;
        
        _ycWindow.hidden = YES;
    }
...
@property (nonatomic , copy ) void (^updateBlock)(YCItem *);
...
@property (nonatomic , copy ) void (^updateBlock)(YCAction *);

@end

@implementation YCAction
...
@interface YCItemView : UIView

@property (nonatomic , strong ) YCItem *item;

+ (YCItemView *)view;

@end

@implementation YCItemView

+ (YCItemView *)view{
    
    return [[YCItemView alloc] init];
}

@end

@interface YCItemLabel : UILabel
...
@implementation YCItemLabel

+ (YCItemLabel *)label{
    
    return [[YCItemLabel alloc] init];
}
...
@interface YCItemTextField : UITextField

@property (nonatomic , strong ) YCItem *item;

+ (YCItemTextField *)textField;

@end

@implementation YCItemTextField

+ (YCItemTextField *)textField{
    
    return [[YCItemTextField alloc] init];
}

@end

@interface YCActionButton : UIButton

@property (nonatomic , strong ) YCAction *action;
...
- (void)setAction:(YCAction *)action{
    ...
    if (action.borderPosition & YCActionBorderPositionTop &&
        action.borderPosition & YCActionBorderPositionBottom &&
        action.borderPosition & YCActionBorderPositionLeft &&
        action.borderPosition & YCActionBorderPositionRight) {
        ...
        if (action.borderPosition & YCActionBorderPositionTop) [self addTopBorder]; else [self removeTopBorder];
        
        if (action.borderPosition & YCActionBorderPositionBottom) [self addBottomBorder]; else [self removeBottomBorder];
        
        if (action.borderPosition & YCActionBorderPositionLeft) [self addLeftBorder]; else [self removeLeftBorder];
        
        if (action.borderPosition & YCActionBorderPositionRight) [self addRightBorder]; else [self removeRightBorder];
    }
    
    __weak typeof(self) weakSelf = self;
    
    action.updateBlock = ^(YCAction *act) {
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _positionType = YCCustomViewPositionTypeCenter;
    }
    return self;
}
...
@property (nonatomic , assign ) YCScreenOrientationType orientationType;

@property (nonatomic , strong ) YCCustomView *customView;
...
- (void)viewDidLoad{
    ...
    if (self.config.modelBackgroundStyle == YCBackgroundStyleBlur) {
    ...
    self.orientationType = VIEW_HEIGHT > VIEW_WIDTH ? YCScreenOrientationTypeVertical : YCScreenOrientationTypeHorizontal;
}
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator{
    ...
    self.orientationType = size.height > size.width ? YCScreenOrientationTypeVertical : YCScreenOrientationTypeHorizontal;
}
...
- (void)closeAnimationsWithCompletionBlock:(void (^)(void))completionBlock{
    ...
    [[YCAlert shareManager].ycWindow endEditing:YES];
}
...
- (UIWindow *)currentKeyWindow{
    ...
    if (!_currentKeyWindow) _currentKeyWindow = [YCAlert shareManager].mainWindow;
    ...
    if (_currentKeyWindow) if (![YCAlert shareManager].mainWindow) [YCAlert shareManager].mainWindow = _currentKeyWindow;
...
@property (nonatomic , strong ) NSMutableArray <YCActionButton *>*alertActionArray;
...
- (CGFloat)updateAlertItemsLayout{
    ...
    [self.alertItemArray enumerateObjectsUsingBlock:^(id  _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([item isKindOfClass:UIView.class]) {
            ...
            YCItemView *view = (YCItemView *)item;
        ...
        } else if ([item isKindOfClass:YCCustomView.class]) {
            
            YCCustomView *custom = (YCCustomView *)item;
    ...
    for (YCActionButton *button in self.alertActionArray) {
    ...
    if (self.alertActionArray.count == 2) {
        ...
        YCActionButton *buttonA = self.alertActionArray.count == self.config.modelActionArray.count ? self.alertActionArray.firstObject : self.alertActionArray.lastObject;
        
        YCActionButton *buttonB = self.alertActionArray.count == self.config.modelActionArray.count ? self.alertActionArray.lastObject : self.alertActionArray.firstObject;
...
- (void)configAlert{
    ...
    [self.config.modelItemArray enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        void (^itemBlock)(YCItem *) = obj;
        
        YCItem *item = [[YCItem alloc] init];
        ...
        switch (item.type) {
            ...
            case YCItemTypeTitle:
            {
                void(^block)(UILabel *label) = item.block;
                
                YCItemLabel *label = [YCItemLabel label];
            ...
            case YCItemTypeCustomView:
            {
                void(^block)(YCCustomView *) = item.block;
                
                YCCustomView *custom = [[YCCustomView alloc] init];
            ...
            case YCItemTypeTextField:
            {
                YCItemTextField *textField = [YCItemTextField textField];
    ...
    [self.config.modelActionArray enumerateObjectsUsingBlock:^(id item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        void (^block)(YCAction *action) = item;
        
        YCAction *action = [[YCAction alloc] init];
        ...
        if (!action.borderPosition) action.borderPosition = (self.config.modelActionArray.count == 2 && idx == 0) ? YCActionBorderPositionTop | YCActionBorderPositionRight : YCActionBorderPositionTop;
        
        if (!action.height) action.height = 45.0f;
        
        YCActionButton *button = [YCActionButton button];
...
- (void)buttonAction:(YCActionButton *)sender{
...
- (void)showAnimationsWithCompletionBlock:(void (^)(void))completionBlock{
    ...
    if (self.config.modelOpenAnimationStyle & YCAnimationStyleOrientationNone) {
    ...
    } else if (self.config.modelOpenAnimationStyle & YCAnimationStyleOrientationTop) {
        
        containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
        
        containerFrame.origin.y = 0 - containerFrame.size.height;
        
    } else if (self.config.modelOpenAnimationStyle & YCAnimationStyleOrientationBottom) {
        
        containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
        
        containerFrame.origin.y = viewHeight;
        
    } else if (self.config.modelOpenAnimationStyle & YCAnimationStyleOrientationLeft) {
        
        containerFrame.origin.x = 0 - containerFrame.size.width;
        
        containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
        
    } else if (self.config.modelOpenAnimationStyle & YCAnimationStyleOrientationRight) {
    ...
    if (self.config.modelOpenAnimationStyle & YCAnimationStyleFade) self.containerView.alpha = 0.0f;
    
    if (self.config.modelOpenAnimationStyle & YCAnimationStyleZoomEnlarge) self.containerView.transform = CGAffineTransformMakeScale(0.6f , 0.6f);
    
    if (self.config.modelOpenAnimationStyle & YCAnimationStyleZoomShrink) self.containerView.transform = CGAffineTransformMakeScale(1.2f , 1.2f);
    ...
    if (self.config.modelOpenAnimationConfigBlock) self.config.modelOpenAnimationConfigBlock(^{
        ...
        if (weakSelf.config.modelBackgroundStyle == YCBackgroundStyleTranslucent) {
        ...
        } else if (weakSelf.config.modelBackgroundStyle == YCBackgroundStyleBlur) {
...
- (void)closeAnimationsWithCompletionBlock:(void (^)(void))completionBlock{
    ...
    if (self.config.modelCloseAnimationConfigBlock) self.config.modelCloseAnimationConfigBlock(^{
        ...
        if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleOrientationNone) {
            
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            
            containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
            
        } else if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleOrientationTop) {
            
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            
            containerFrame.origin.y = 0 - containerFrame.size.height;
            
        } else if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleOrientationBottom) {
            
            containerFrame.origin.x = (viewWidth - containerFrame.size.width) * 0.5f;
            
            containerFrame.origin.y = viewHeight;
            
        } else if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleOrientationLeft) {
            
            containerFrame.origin.x = 0 - containerFrame.size.width;
            
            containerFrame.origin.y = (viewHeight - containerFrame.size.height) * 0.5f;
            
        } else if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleOrientationRight) {
        ...
        if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleFade) weakSelf.containerView.alpha = 0.0f;
        
        if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleZoomEnlarge) weakSelf.containerView.transform = CGAffineTransformMakeScale(1.2f , 1.2f);
        
        if (weakSelf.config.modelCloseAnimationStyle & YCAnimationStyleZoomShrink) weakSelf.containerView.transform = CGAffineTransformMakeScale(0.6f , 0.6f);
...
- (NSMutableArray <YCActionButton *>*)alertActionArray{
...
@interface YCActionSheetViewController ()
...
@property (nonatomic , strong ) NSMutableArray <YCActionButton *>*actionSheetActionArray;

@property (nonatomic , strong ) UIView *actionSheetCancelActionSpaceView;

@property (nonatomic , strong ) YCActionButton *actionSheetCancelAction;

@end

@implementation YCActionSheetViewController
{
    BOOL isShowed;
}
...
- (void)updateActionSheetLayoutWithViewWidth:(CGFloat)viewWidth ViewHeight:(CGFloat)viewHeight{
    ...
    for (YCActionButton *button in self.actionSheetActionArray) {
...
- (void)configActionSheet{
    ...
    for (id item in self.config.modelActionArray) {
        ...
        switch (action.type) {
            ...
            default:
            {
                if (!action.borderPosition) action.borderPosition = YCActionBorderPositionTop;
...
- (void)showAnimationsWithCompletionBlock:(void (^)(void))completionBlock{
    ...
    if (self.config.modelOpenAnimationConfigBlock) self.config.modelOpenAnimationConfigBlock(^{
        ...
        switch (weakSelf.config.modelBackgroundStyle) {
            ...
            case YCBackgroundStyleBlur:
            {
                weakSelf.backgroundVisualEffectView.effect = [UIBlurEffect effectWithStyle:weakSelf.config.modelBackgroundBlurEffectStyle];
            }
                break;
                
            case YCBackgroundStyleTranslucent:
            {
...
- (NSMutableArray <YCActionButton *>*)actionSheetActionArray{
    ...
    if (self) {
        ...
        self.config.modelFinishConfig = ^{
            ...
            if ([YCAlert shareManager].queueArray.count) {
                ...
                if (!last.config.modelIsQueue && last.config.modelQueuePriority <= strongSelf.config.modelQueuePriority) [[YCAlert shareManager].queueArray removeObject:last];
                
                if (![[YCAlert shareManager].queueArray containsObject:strongSelf]) {
                    
                    [[YCAlert shareManager].queueArray addObject:strongSelf];
                    
                    [[YCAlert shareManager].queueArray sortUsingComparator:^NSComparisonResult(YCAlertConfig *configA, YCAlertConfig *configB) {
                ...
                if ([YCAlert shareManager].queueArray.lastObject == strongSelf) [strongSelf show];
...
- (void)show{
    ...
    [YCAlert shareManager].viewController.config = self.config;
    
    [YCAlert shareManager].ycWindow.rootViewController = [YCAlert shareManager].viewController;
    ...
    [YCAlert shareManager].viewController.openFinishBlock = ^{
        
    };
    
    [YCAlert shareManager].viewController.closeFinishBlock = ^{
        
        __strong typeof(weakSelf) strongSelf = weakSelf;
        
        if (!strongSelf) return;
        
        if ([YCAlert shareManager].queueArray.lastObject == strongSelf) {
            
            [YCAlert shareManager].ycWindow.hidden = YES;
            
            [[YCAlert shareManager].ycWindow resignKeyWindow];
            
            [YCAlert shareManager].ycWindow.rootViewController = nil;
            
            [YCAlert shareManager].viewController = nil;
            
            [[YCAlert shareManager].queueArray removeObject:strongSelf];
            
            if (strongSelf.config.modelIsContinueQueueDisplay) [YCAlert continueQueueDisplay];
...
- (void)closeWithCompletionBlock:(void (^)(void))completionBlock{
    ...
    if ([YCAlert shareManager].viewController) [[YCAlert shareManager].viewController closeAnimationsWithCompletionBlock:completionBlock];
}
